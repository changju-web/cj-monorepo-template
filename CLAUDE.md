...
这是基于 **vue-pure-admin** 精简版的企业级前端管理系统脚手架，适用于构建大型管理后台应用。
...
- **静态路由**: 位于 `src/router/modules/` 目录下，使用 Vite 的 `import.meta.glob` 自动导入
- **动态路由**: 通过后端接口返回，在 `src/router/utils.ts` 的 `initRouter()` 中处理
- **路由扁平化**: 三级及以上路由会被拍平为二级路由（`formatTwoStageRoutes`）
- **路由白名单**: 配置在 `src/router/index.ts`，默认包含 `/login`

**重要**: 新增静态路由时，只需在 `src/router/modules/` 目录下创建 `.ts` 文件并导出路由配置，无需手动引入。

### 状态管理 (Pinia)

状态模块位于 `src/store/modules/`：
...
- 使用 Axios 进行请求封装
- 请求拦截器位于 `src/utils/http/index.ts`
...
3. **路由守卫**: 登录权限和动态路由加载在 `src/router/index.ts` 的 `router.beforeEach` 中处理
